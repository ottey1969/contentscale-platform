console.log('%c🚀 ContentScale v2.1 - WITH RESULTS MODAL & PENALTY SYSTEM','color:#8b5cf6;font-size:16px;font-weight:bold');
...
class PenaltySystem {
    ...
    detectPageType(url) {
        ...
        if (urlLower.includes('/admin') || urlLower.includes('/wp-admin') || urlLower.includes('/backend')) {
            return 'ADMIN';
        }
        if (urlLower.includes('/login') || urlLower.includes('/signin') || urlLower.includes('/wp-login')) {
            return 'LOGIN';
        }
        if (urlLower.includes('/dashboard') || urlLower.includes('/panel') || urlLower.includes('/control-panel')) {
            return 'DASHBOARD';
        }
        if (urlLower.includes('/thank-you') || urlLower.includes('/success') || urlLower.includes('/confirmation')) {
            return 'THANK_YOU';
        }
    ...
    applyPenalties(scanResult) {
        const url = scanResult.url || '';
        const pageType = this.detectPageType(url);
        
        // Get raw scores from API
        const graafRaw = scanResult.breakdown?.graaf?.total || 0;
        const craftRaw = scanResult.breakdown?.craft?.total || 0;
        const technicalRaw = scanResult.breakdown?.technical?.total || 0;
        
        // Calculate raw total
        const rawTotal = graafRaw + craftRaw + technicalRaw;
        ...
        if (!scanResult.meta || !scanResult.meta.description || scanResult.meta.description.length < 50) {
            totalPenalties += this.criticalPenalties.NO_META_DESCRIPTION;
            penaltyDetails.push({
                title: 'Missing or Poor Meta Description',
                points: this.criticalPenalties.NO_META_DESCRIPTION,
                impact: 'HIGH'
            });
        }
        
        // Check for missing H1
        if (!scanResult.headings || !scanResult.headings.h1 || scanResult.headings.h1.length === 0) {
            totalPenalties += this.criticalPenalties.NO_H1;
            penaltyDetails.push({
                title: 'Missing H1 Heading',
                points: this.criticalPenalties.NO_H1,
                impact: 'HIGH'
            });
        }
        
        // Check for admin/login pages
        if (pageType === 'ADMIN' || pageType === 'LOGIN' || pageType === 'DASHBOARD') {
            totalPenalties += this.criticalPenalties.ADMIN_PAGE;
            penaltyDetails.push({
                title: `${pageType} Page (Not meant for SEO)`,
                points: this.criticalPenalties.ADMIN_PAGE,
                impact: 'HIGH'
            });
        }
        
        // Check for thin content
        if (scanResult.content && scanResult.content.wordCount < 300) {
            totalPenalties += this.criticalPenalties.THIN_CONTENT;
            penaltyDetails.push({
                title: 'Thin Content (<300 words)',
                points: this.criticalPenalties.THIN_CONTENT,
                impact: 'HIGH'
            });
        }
        
        // Check for no schema
        if (!scanResult.schema || scanResult.schema.length === 0) {
            totalPenalties += this.criticalPenalties.NO_SCHEMA;
            penaltyDetails.push({
                title: 'No Schema Markup',
                points: this.criticalPenalties.NO_SCHEMA,
                impact: 'MEDIUM'
            });
        }
        
        // Apply page type cap
        const maxCap = this.pageTypeCaps[pageType] || 100;
        
        // Calculate score after penalties
        const scoreAfterPenalties = Math.max(0, rawTotal - totalPenalties);
        const finalScore = Math.min(scoreAfterPenalties, maxCap);
        
        // Scale individual scores proportionally
        const scaleFactor = finalScore / Math.max(1, rawTotal);
        ...
        return {
            ...
            maxCap: maxCap,
            adjustedBreakdown: {
                graaf: Math.round(graafRaw * scaleFactor),
                craft: Math.round(craftRaw * scaleFactor),
                technical: Math.round(technicalRaw * scaleFactor)
            }
        };
    }
}
...
async function performScan() {
  ...
  try {
    ...
    // ✅ APPLY PENALTIES
    const penaltyResult = penaltySystem.applyPenalties(result);
    console.log('[PENALTIES] Applied:', penaltyResult);
    
    // ✅ MERGE PENALTY DATA
    result.penaltyData = penaltyResult;
    result.originalScore = result.score;
    result.score = penaltyResult.finalScore;
    
    // ✅ ADJUST BREAKDOWN SCORES
    if (result.breakdown) {
      result.breakdown.graaf.total = penaltyResult.adjustedBreakdown.graaf;
      result.breakdown.craft.total = penaltyResult.adjustedBreakdown.craft;
      result.breakdown.technical.total = penaltyResult.adjustedBreakdown.technical;
    }
...
function generateModalContent(result) {
  ...
  const penaltyData = result.penaltyData;
  ...
  return `
    ...
    <div class="modal-header">
      ...
      ${penaltyData && penaltyData.pageType !== 'CONTENT' ? `
      <div class="mt-3 inline-block px-4 py-1 rounded-full text-sm font-bold" style="background: #ef4444; color: white;">
        ⚠️ ${penaltyData.pageType} PAGE
      </div>
      ` : ''}
    ...
    <div class="modal-body">
      ...
      <!-- PENALTY DISPLAY -->
      ${penaltyData && penaltyData.penalties > 0 ? `
      <div class="bg-red-900 bg-opacity-20 rounded-lg p-5 mb-8 border border-red-700">
        <h4 class="text-xl font-bold mb-3 text-red-300">⚠️ Penalties Applied</h4>
        <div class="space-y-2 mb-3">
          <div class="flex justify-between items-center">
            <span class="text-gray-300">Original Score:</span>
            <span class="text-xl font-bold">${penaltyData.rawScore}/100</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-red-300">Total Penalties:</span>
            <span class="text-xl font-bold text-red-400">-${penaltyData.penalties}</span>
          </div>
          <div class="flex justify-between items-center border-t border-red-800 pt-2">
            <span class="text-white font-bold">Final Score:</span>
            <span class="text-2xl font-bold">${score}/100</span>
          </div>
        </div>
        ${penaltyData.penaltyDetails && penaltyData.penaltyDetails.length > 0 ? `
        <div class="mt-4">
          <div class="text-sm text-red-300 mb-2">Penalty Details:</div>
          <ul class="text-sm text-gray-300 space-y-1">
            ${penaltyData.penaltyDetails.map(p => `
              <li class="flex justify-between">
                <span>• ${p.title}</span>
                <span class="text-red-400">-${p.points}</span>
              </li>
            `).join('')}
          </ul>
        </div>
        ` : ''}
        ${penaltyData.maxCap < 100 ? `
        <div class="mt-4 p-3 bg-red-900 bg-opacity-30 rounded border border-red-800">
          <div class="text-sm text-red-300">ℹ️ ${penaltyData.pageType} page - SEO score capped at ${penaltyData.maxCap}/100</div>
        </div>
        ` : ''}
      </div>
      ` : ''}
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('✅ ContentScale initialized with results modal & penalty system');
