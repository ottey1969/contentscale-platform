...
app.post('/api/gemini-proxy', async (req, res) => {
  ...
  try {
    ...
    // Add Google Search grounding so Gemini can actually browse the web for leads
    const body = { ...req.body };
    if (!body.tools) {
...
// ── Otto AI Client JS — served inline ─────────────────────────────────────
...
app.get('/gemini-live-client-v6.js', (req, res) => {
  res.setHeader('Content-Type', 'application/javascript; charset=utf-8');
  res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.send(OTTO_CLIENT_JS);
});
app.get('/gemini-live-client.js', (req, res) => {
  res.setHeader('Content-Type', 'application/javascript; charset=utf-8');
  res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
...
app.get('/api/otto-version', (req, res) => {
  res.json({ version: 'v6', model: 'gemini-3.1-flash-live-preview', voice: 'Fenrir', timestamp: new Date().toISOString() });
});
