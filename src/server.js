...
async function createAllTables() {
  ...
  try {
    ...
    // Users table - MET EXTRA KOLOMMEN VOOR ADMIN MANAGEMENT
    await client.query(`
...
// ============================================
// ✅ ADMIN USER MANAGEMENT ENDPOINTS - NIEUW!
// ============================================
...
// POST add user
app.post('/api/admin/users/add', verifyAdmin, async (req, res) => {
  const { user_id, email, name, activation_days } = req.body;
  
  try {
    const expiryDate = new Date();
    expiryDate.setDate(expiryDate.getDate() + activation_days);
    
    await pool.query(
      `INSERT INTO users (id, email, name, is_activated, activation_expires)
      VALUES ($1, $2, $3, TRUE, $4)
      ON CONFLICT (id) DO UPDATE SET 
      is_activated = TRUE, activation_expires = $4, name = $3, email = $2`,
      [user_id, email, name, expiryDate]
    );
    
    console.log(`✅ User ${name} activated for ${activation_days} days`);
    res.json({ success: true });
  } catch (error) {
...
app.post('/api/admin/users/:id/toggle-activation', verifyAdmin, async (req, res) => {
  const { id } = req.params;
  const { activate, days } = req.body;
  
  try {
    if (activate && days) {
      const expiryDate = new Date();
      expiryDate.setDate(expiryDate.getDate() + days);
      await pool.query('UPDATE users SET is_activated = TRUE, activation_expires = $1 WHERE id = $2', [expiryDate, id]);
      console.log(`✅ User ${id} activated for ${days} days`);
    } else {
...
async function startServer() {
  ...
  app.listen(PORT, () => {
    ...
    console.log('   • Admin User Management: ✅ NIEUW TOEGEVOEGD');
