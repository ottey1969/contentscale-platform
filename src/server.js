...
// ── Otto AI client JS routes ──────────────────────────────────────────────
const _fs = require('fs');
const _path = require('path');
const _ottoJsPaths = [
  _path.join(__dirname, 'gemini-live-client-v6.js'),
  _path.join(__dirname, 'public', 'gemini-live-client-v6.js'),
];
let _ottoJsContent = null;
function getOttoJs() {
  if (_ottoJsContent) return _ottoJsContent;
  for (const p of _ottoJsPaths) {
    if (_fs.existsSync(p)) { _ottoJsContent = _fs.readFileSync(p, 'utf8'); return _ottoJsContent; }
  }
  return '// gemini-live-client-v6.js not found on server';
}
